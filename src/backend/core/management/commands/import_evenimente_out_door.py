...
JSON_PATH = "/mnt/c/Users/Maxim/Downloads/outdoor_events_page_content.json"
PLACEHOLDER = "placeholder.png"


class Command(BaseCommand):
    help = "Import Outdoor Events page content from JSON"

    def handle(self, *args, **options):
        with open(JSON_PATH, encoding="utf-8") as f:
            data = json.load(f)

        page_data = data["outdoor_events_page"]

        page, created = Page.objects.get_or_create(
            slug="evenimente-out-door",
            defaults={"id": uuid.uuid4()},
        )
        page.name_ro = "Evenimente Out Door"
        page.name_en = "Outdoor Events"
        page.name_ru = "Мероприятия На Открытом Воздухе"
        ...
        self.stdout.write(self.style.SUCCESS(f"Imported Outdoor Events page (slug: {page.slug})"))
